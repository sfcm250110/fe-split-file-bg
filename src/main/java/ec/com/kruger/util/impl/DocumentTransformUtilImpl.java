...
public class DocumentTransformUtilImpl implements DocumentTransformUtil, Serializable {
	...
	public static ComprobanteRetencionAts obtenerComprobanteRetencionAts(String pComprobanteRetencion) throws Exception {
		...
		List<RetencionAts> retencionesLista = new ArrayList<RetencionAts>();
		List<ImpuestoAts> impuestosLista = new ArrayList<ImpuestoAts>();
		...
		for (String linea : lineas) {
			...
			if (tipoLinea.equals(TIPO_LINEA_CABECERA_COMPROBANTE)) {
				...
				ImpuestoAts impuesto = ComprobanteRetencionBuildUtil.getImpuesto(linea);
				impuestosLista.add(impuesto);

			} else if (tipoLinea.equals(TIPO_LINEA_DET)) {
				DocumentoSustentoComprobanteRetencionAts detalle = ComprobanteRetencionBuildUtil.getDocumentoSustentoComprobanteRetencion(linea);
				documentoSustento.add(detalle);

			} else if (tipoLinea.equals(TIPO_LINEA_RET)) {
				RetencionAts retencion = ComprobanteRetencionBuildUtil.getRetencion(linea);
				retencionesLista.add(retencion);

			} else if (tipoLinea.equals((TIPO_LINEA_ADICIONAL))) {
				InformacionAdicionalComprobanteRetencionAts informacionAdicional = ComprobanteRetencionBuildUtil.getInformacionAdicionalComprobanteRetencion(linea);
				informacionesAdicional.add(informacionAdicional);
			}
		}
		...
		try {
			...
			comprobanteRetencion.setRetencionesLista(retencionesLista);
